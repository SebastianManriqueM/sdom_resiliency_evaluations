...
foreach ($tag in $tags) {
    ...
    $logFile = "$RR\logs\rerun_$tag.log"
    uv run python "$RR\run_resiliency_evaluation.py" *>&1 | Tee-Object -FilePath $logFile
    $tagEnd = Get-Date
...
uv run python "$RR\make_sweep_summary.py" *>&1 | Tee-Object -FilePath "$RR\logs\rerun_sweep_summary.log"
